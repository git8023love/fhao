$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/salesupply/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function () {
			var url = vm.saleSupply.id == null ? "sys/salesupply/save" : "sys/salesupply/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function () {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "sys/salesupply/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/salesupply/info/"+id, function(r){
