...
import io.fhao.common.Constant.MenuType;
...
import io.fhao.modules.sys.service.SysMenuService;
...
public class SysMenuController extends AbstractController {
	...
	private void verifyForm(SysMenu menu){
		if(StringUtils.isBlank(menu.getName())){
			throw new FhaoException("菜单名称不能为空");
		}
		
		if(menu.getParentId() == null){
			throw new FhaoException("上级菜单不能为空");
		}
		
		//菜单
		if(menu.getType() == MenuType.MENU.getValue()){
			if(StringUtils.isBlank(menu.getUrl())){
				throw new FhaoException("菜单URL不能为空");
			}
		}
		...
		if(menu.getType() == MenuType.CATALOG.getValue() ||
			...
			if(parentType != MenuType.CATALOG.getValue()){
				throw new FhaoException("上级菜单只能为目录类型");
			}
			return ;
		}
		...
		if(menu.getType() == MenuType.BUTTON.getValue()){
			if(parentType != MenuType.MENU.getValue()){
				throw new FhaoException("上级菜单只能为菜单类型");
			}
			return ;
		}
	}
